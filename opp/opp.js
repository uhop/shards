...
(function(){
    dojo.declare("shards.opp.Parser", null, {
        ...
        constructor: function(init, table, brackets){
            ...
            dojo.forEach(table, function(item){
                ...
                this[item.before][item.name] = item;
            }, this);
            //assert(this[init]);
        ...
        supply: function(token){
            ...
            var newState = this[this.expected][token.name];
            if(!newState){
                throw Error("Unknown input symbol: " + token.name);
            }
    ...
    shards.opp.getNextTerm = function getNextTerm(parser, scanner){
    ...
    shards.opp.putNextToken = function putNextToken(parser, token, interpreter){
    ...
    shards.opp.parse = function parse(parser, scanner, interpreter){
        var putNextToken = shards.opp.putNextToken;
        while(putNextToken(parser, scanner.getToken(), interpreter)){}
    }
})();
